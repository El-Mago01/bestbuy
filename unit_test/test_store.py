...
"""
ORIGINAL_PRODUCT_LIST = [products.Product("MacBook Air M2", price=1450, quantity=100),
...
def test_11_store_creation(default_ajax_store):
    ...
    assert default_ajax_store.get_all_products()[3].get_promotion() == thirty_percent
...
products.LimitedProduct("Shipping", price=10, quantity=250, maximum=1)
"""
def test_23_buy_product_from_store_via_shopping_list_no_promotion(default_ajax_store):
...
def test_24_buy_product_with_various_product_promotions(discount_ajax_store):
    ...
    assert price == (10*1450*0.7) + (5*250+5*125) + (7*500) + (10*125*0.7) + 10*.7
# 2.5
def test_25_buy_product_from_store_via_empty_shopping_list1(default_ajax_store):
    shopping_list = []
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)
#2.6
def test_26_buy_product_from_store_via_empty_shopping_list2(default_ajax_store):
    shopping_list = [()]
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)
#2.7
def test_27_buy_product_from_store_via_error_shopping_list3(default_ajax_store):
    shopping_list = 4
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)
#2.8
def test_28_buy_product_from_store_via_error_shopping_list3(default_ajax_store):
    shopping_list = [("hello", 4)]
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)

#2.9
def test_29_buy_product_from_store_via_error_shopping_list4(default_ajax_store):
    shopping_list = [(default_ajax_store.get_all_products()[0], "hello")]
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)

#2.10
def test_210_buy_product_from_store_via_error_shopping_list5(default_ajax_store):
    shopping_list = [(default_ajax_store.get_all_products()[0], 10), ("hello", 10)]
    with pytest.raises(TypeError, match=r"Shopping list must be a list of '\(product, quantity\)' pairs"):
        price = default_ajax_store.order(shopping_list)
#3.1
def test_31_product_in_store(default_ajax_store):
    assert ("Mac" in default_ajax_store) == True
    assert ("Earbuds" in default_ajax_store) == True
